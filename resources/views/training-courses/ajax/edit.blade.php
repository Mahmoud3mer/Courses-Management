...
@section('content')
    <div class="col-12" style="display: flex; justify-content: center; align-items: center;">
        <form id="form-create-course" style="width:80%; background:white; padding:20px; border-radius:10px;" method="POST"
            action="{{ route('training-courses.update', $trainingCourse->id) }}">
            @csrf
            @method('PUT')
                ...
                <select class="custom-select" style="width: 100%;" id="course_id" name="course_id"
                    ...
                    @isset($courses)
                        @foreach ($courses as $course)
                            <option value="{{ $course->id  }}" @if (old('course_id', $trainingCourse->course_id) == $course->id) selected @endif>
                                {{ $course->name }}</option>
                        @endforeach
                    @endisset
                </select>
            ...
            <div class="mb-4">
                ...
                <input autofocus type="number" min="0" class="form-control" id="price" name="price"
                    value="{{ old('price', $trainingCourse->price) }}" placeholder="أدخل سعر الدورة">
                @error('price')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" class="form-control" id="start_date" name="start_date" value="{{ old('start_date', $trainingCourse->start_date) }}" placeholder="أدخل تاريخ بدء الدورة">
                @error('start_date')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input autofocus type="date" class="form-control" id="end_date" name="end_date"
                    value="{{ old('end_date', $trainingCourse->end_date) }}" placeholder="أدخل تاريخ انتهاء الدورة">
                @error('end_date')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control" id="note" name="note" rows="3"
                    placeholder="أدخل ملاحظات">{{ old('note', $trainingCourse->note) }}</textarea>
            </div>

            <button type="submit" class="btn btn-primary">تحديث الدورة التدريبية</button>
        </form>
    </div>
@endsection
